...
| Live public check | `tests/scripts.live-public-check.test.mjs` | Source pins that `scripts/live-public-check.sh` is read-only and asserts live resolver chrome + browser API origin on `/` and `/id`. |
