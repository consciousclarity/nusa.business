...
| Onboarding / claims | `tests/api.onboarding-claims.test.mjs` | Owner self-register, invite register, safe returnTo, duplicate pending claims, pending claim does not grant PATCH, audited decisions, recovery tokens. |
...
| Live public check | `tests/scripts.live-public-check.test.mjs` | Source pins that `scripts/live-public-check.sh` is read-only and asserts live resolver chrome, `/id` island names, listing `/id` category/address/weekday chrome, and browser API origin. |
...
| Public SEO helpers | `tests/web.seo.test.mjs` | Canonical origin, `/host` paths, LocalBusiness JSON-LD (locale `additionalType`), `/id` sitemap paths, sitemap XML escaping. |
