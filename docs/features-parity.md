...
| Search / filters | partial | API `/v1/search` + public `/search?q=` via unified `DirectorySearch` on nation, hub, place, listing, and search; `/c/{category}` browse facets stay separate; allowlisted `/c/{category}/{facet}/{value}` index ([ADR-006](architecture/adr/006-indexable-facet-urls.md)) |
